...
`POST /api/evaluation` runs the same batch, publishes the run through `EvaluationRunStore`, and persists the Recovery Cases it drove — so the batch's cases are part of the live projection rather than a parallel set of numbers, and the run keeps the ground-truth scoring only a seeded batch can know: diagnosis accuracy, unsafe and duplicate actions prevented, and each case's expected safe action and outcome. A published run survives a restart; `GET /api/evaluation` replays it without re-running the batch. This MVP has a single synthetic merchant, so seeded cases are the merchant's cases — nothing distinguishes a demo case from a live one, and nothing needs to.
