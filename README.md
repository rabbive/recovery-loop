...
- **Recurring retry.** It reads the original payment (`GET /v1/payments/:id`) and requires a complete mandate token, customer, email, and contact. An explicit negative `recurring` flag refuses the charge; the undocumented absence of that field does not. Before any POST, the provider customer and original order must equal the registered case, and the subscription id in the payment or original-order notes must match. It then creates a new order (`POST /v1/orders`) whose `receipt` is the action identity and charges the mandate (`POST /v1/payments/create/recurring`) for the case's amount and currency. The order and charge notes carry `caseId`, `subscriptionId`, and `recoveryActionKey`. The response only means *submitted*; the webhook decides the outcome.
